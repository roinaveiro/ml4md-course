...
def get_isosmiles(mol) -> str:
    return Chem.MolToSmiles(mol, isomericSmiles=True, canonical=True)


def get_mfp(mol) -> np.ndarray:
...
def is_single(mol) -> bool:
    smi = get_isosmiles(mol)
    return len(Chem.GetMolFrags(mol)) == 1 or not ('.' in smi)


def is_larger_molecule(mol) -> bool:
    return mol.GetNumAtoms() > 1


def get_atom_set(mols, add_null: bool = True) -> List[str]:
